...
public record NoiseGeneratorSettings(NoiseSettings noiseSettings, BlockState defaultBlock, BlockState defaultFluid, NoiseRouter noiseRouter, SurfaceRules.RuleSource surfaceRule, List<Climate.ParameterPoint> spawnTarget, int seaLevel, boolean disableMobGeneration, boolean aquifersEnabled, boolean oreVeinsEnabled, boolean useLegacyRandomSource) {
   ...
   public static Holder<NoiseGeneratorSettings> register(Registry<NoiseGeneratorSettings> pRegistry, ResourceKey<NoiseGeneratorSettings> pKey, NoiseGeneratorSettings pSettings) {
      return BuiltinRegistries.register(pRegistry, pKey.location(), pSettings);
   }

   public static Holder<NoiseGeneratorSettings> bootstrap(Registry<NoiseGeneratorSettings> pRegistry) {
      register(pRegistry, OVERWORLD, overworld(false, false));
      register(pRegistry, LARGE_BIOMES, overworld(false, true));
      register(pRegistry, AMPLIFIED, overworld(true, false));
      register(pRegistry, NETHER, nether());
      register(pRegistry, END, end());
      register(pRegistry, CAVES, caves());
      return register(pRegistry, FLOATING_ISLANDS, floatingIslands());
   }
